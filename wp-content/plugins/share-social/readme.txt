...
IMPORTANT 2.2.0 CHANGES: Due to high server running costs Cunjo.com will no longer stream the widgets. The widget constructor script is now Open Source, built in the plugin. So the widgets created with this plugin are now streamed from your website, no longer from Cunjo.com servers. Social analytics will remain a Cunjo.com service since the platform is to complex to implement within the plugin. We have tested the new plugin and streaming configurations from a low end server and the response times are good, since the streaming is asynch it does not increase your initial website load times. Enjoy

PS: You will need a FREE Cunjo ID to use it. Once you generate the ID from your Cunjo General Settings, check your email and copy the Cunjo ID from the welcome email received. Paste the code in the General Settings form and save.
