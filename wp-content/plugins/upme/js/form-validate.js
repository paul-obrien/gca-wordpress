...
jQuery(document).ready(function() {
    ...
    var err_messages={
        ...
        "slug-exists"           : Validate.ErrMsg.slugexists,
        "invalid-slug"          : Validate.ErrMsg.sluginvalid
    ...
    /* Edit Profile Form: Validate username on focus out */
    var slug_class = '.upme-edit-slug';

    jQuery(slug_class).focus(function(){
        jQuery(this).removeClass('error');
    });

    jQuery(slug_class).blur(function(){
        ...
        jQuery(slug).removeClass('error');

        jQuery("#upme-reg-slug-img").remove();
        jQuery("#upme-reg-slug-msg").remove();

        if(!slug_reg.test(newSlug)){
            message = Validate.ErrMsg.sluginvalid;
            jQuery(slug).addClass('error');
            jQuery(slug).after('<div id="upme-reg-slug-msg" class="upme-input-text-inline-error" ><i id="upme-reg-slug-img" original-title="Invalid" class="upme-icon-remove upme-input-text-font-cancel" ></i>' + message + '</div>');
               
        } else{
            jQuery.post(
                UPMECustom.AdminAjax,
                {
                    'action': 'upme_validate_edit_profile_slug',
                    'slug':   newSlug,
                    'user_id' : user_id
                },
                function(response){
                    ...
                    switch(response.msg){
                        case 'RegExistSlug':
                            message = Validate.ErrMsg.slugexists; 
                            break;
                        case 'RegValidSlug':
                            message = Validate.ErrMsg.slugvalid; 
                            break;
                    }
                    ...
                    if(response.status){
                        ...
                        jQuery(slug).after('<div id="upme-reg-slug-msg" class="upme-input-text-inline-success" ><i id="upme-reg-slug-img" original-title="Valid" class="upme-icon-ok upme-input-text-font-accept" ></i>' + message + '</div>');
                    }
    ...
    jQuery('#upme-reset-password-form').submit(function(e){
        ...
        else
        {
            ...
            if(jQuery("#upme_new_password").val() == '') {
                err = true;
                err_msg+='<span class="upme-error upme-error-block"><i class="upme-icon-remove"></i> New password is required.</span>';
            ...
            if(jQuery("#upme_confirm_new_password").val() == '') {
                err = true;
                err_msg+='<span class="upme-error upme-error-block"><i class="upme-icon-remove"></i> Confirm new password is required.</span>';
...
function upmeValidatePasswordStrength(passField,confirmPassField,usernameField){
    ...
    if(strength != '5'){
        ...
        if(passwordStrengthLevel == "0"){
            ...
            err = true;
            err_msg+='<span class="upme-error upme-error-block"><i class="upme-icon-remove"></i><strong>'+Validate.Err+':</strong> '+ Validate.MinPassStrength + ' ' + messages[statusText[passwordStrengthLevel]] + '</span>';
