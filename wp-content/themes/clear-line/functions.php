...
function get_short_url()
{
	...
	$login = $options[$shortname.'_bitly_login'];
	$appkey = $options[$shortname.'_bitly_api_key'];
	if (!$login || !$appkey) return $long_url;
	$format = 'txt';
	$version = '';
	$bitly = 'http://api.bit.ly/v3/shorten?longUrl='.urlencode($long_url).'&login='.$login.'&format='.$format.'&apiKey='.$appkey;
	...
	if (is_page() || is_single())
	{
		...
		else
		{
			...
			if (!$url) $url = sprintf('%s?p=%s', home_url(),$postid);
